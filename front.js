...
jQuery(document).ready(function($) {
	...
	var anchorEl = '';
	var anchorEl_index = 0;	
	var scrollTop = 0;
	var anchorEls = new Array();
	$('.toc_widget_list a').each(function(){
        anchorEls.push($(this).attr('href'));  //all anchor element
    })
    $('.toc_widget_list li').find('ul').hide(); //all child element hide
	$win.on('scroll',function(){	
		anchorEls.forEach(function(value,index){
			if(!anchorEl){				
				anchorEl = value;						
			}		
			var anchorElOffsetTop = $(anchorEl).offset().top;
			var anchorElOuterHeight = $(anchorEl).outerHeight(true);
			var winScrollTop = $win.scrollTop();

			//down
			if(winScrollTop>=scrollTop && !(winScrollTop > anchorElOffsetTop+anchorElOuterHeight) && !(winScrollTop < anchorElOffsetTop-winHeight)){
				$('.toc_widget_list a').removeClass('active');

				$('a[href="'+anchorEl+'"]').addClass('active').parents('ul').show();
		    //up
			}else if(winScrollTop<scrollTop && !(winScrollTop > anchorElOffsetTop+anchorElOuterHeight) && (winScrollTop < anchorElOffsetTop-winHeight)){
			    $('.toc_widget_list a').removeClass('active');

				anchorEl_index = $.inArray(anchorEl,anchorEls);
				$('a[href="'+anchorEls[anchorEl_index-1]+'"]').addClass('active').siblings('ul').show();
			}else{
				anchorEl = '';
			}
        ...
        //all non-active child element hide
