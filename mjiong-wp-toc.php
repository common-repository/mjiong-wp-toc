<?php
/*
Plugin Name:	MJIONG WP-TOC
Plugin URI: 	https://wordpress.org/plugins/mjiong-wp-toc/
Description: 	A powerful plugin that automatically creates a table of contents. Based on Table of Contents Plus
Author: 		lzw22
Author URI: 	https://lzwdot.com/
Text Domain:	mjiong-wp-toc
Domain Path:	/languages
Version: 		1.0.2
License:		GPL2
*/

/*  Copyright 2018 lzw22  lzw22@qq.com    
...
if ( !class_exists( 'toc' ) ) :
	class toc {
		...
		{
			if ( $file == 'mjiong-wp-toc/' . basename(__FILE__) ) {
				$settings_link = '<a href="options-general.php?page=mjiong-wp-toc">' . __('Settings', 'mjiong-wp-toc') . '</a>';
		...
		{
			load_plugin_textdomain( 'mjiong-wp-toc', false, dirname(plugin_basename(__FILE__)) . '/languages/' );
		}
		...
		function admin_menu()
		{
			$page = add_submenu_page(
				'options-general.php', 
				__('MJIONG WP-TOC', 'mjiong-wp-toc'), 
				__('MJIONG WP-TOC', 'mjiong-wp-toc'), 
				'manage_options', 
				'mjiong-wp-toc', 
		...
		function admin_options() 
		{
			...
			if ( isset( $_GET['update'] ) ) {
				if ( $this->save_admin_options() )
					$msg = '<div id="message" class="updated fade"><p>' . __('Options saved.', 'mjiong-wp-toc') . '</p></div>';
				else
					$msg = '<div id="message" class="error fade"><p>' . __('Save failed.', 'mjiong-wp-toc') . '</p></div>';
			}
...
<ul id="tabbed-nav">
	<li><a href="#tab1"><?php _e('Main Options', 'mjiong-wp-toc'); ?></a></li>
	<li><a href="#tab2"><?php _e('Sitemap', 'mjiong-wp-toc'); ?></a></li>
	<li class="url"><a href="http://mjiong.com/archives/18069.html"><?php _e('Help', 'mjiong-wp-toc'); ?></a></li>
</ul>
...
<tbody>
<tr>
	<th><label for="position"><?php _e('Position', 'mjiong-wp-toc'); ?></label></th>
	<td>
		<select name="position" id="position">
			<option value="<?php echo TOC_POSITION_BEFORE_FIRST_HEADING; ?>"<?php if ( TOC_POSITION_BEFORE_FIRST_HEADING == $this->options['position'] ) echo ' selected="selected"'; ?>><?php _e('Before first heading (default)', 'mjiong-wp-toc'); ?></option>
			<option value="<?php echo TOC_POSITION_AFTER_FIRST_HEADING; ?>"<?php if ( TOC_POSITION_AFTER_FIRST_HEADING == $this->options['position'] ) echo ' selected="selected"'; ?>><?php _e('After first heading', 'mjiong-wp-toc'); ?></option>
			<option value="<?php echo TOC_POSITION_TOP; ?>"<?php if ( TOC_POSITION_TOP == $this->options['position'] ) echo ' selected="selected"'; ?>><?php _e('Top', 'mjiong-wp-toc'); ?></option>
			<option value="<?php echo TOC_POSITION_BOTTOM; ?>"<?php if ( TOC_POSITION_BOTTOM == $this->options['position'] ) echo ' selected="selected"'; ?>><?php _e('Bottom', 'mjiong-wp-toc'); ?></option>
		</select>
	</td>
</tr>
<tr>
	<th><label for="start"><?php _e('Show when', 'mjiong-wp-toc'); ?></label></th>
	<td>
...
?>
		...
		_e('or more headings are present', 'mjiong-wp-toc'); ?>
	</td>
</tr>
<tr>
	<th><?php _e('Auto insert for the following content types', 'mjiong-wp-toc'); ?></th>
	<td><?php
...
<tr>
	...
	_e('Heading text', 'mjiong-wp-toc'); ?></label></th>
	<td>
		<input type="checkbox" value="1" id="show_heading_text" name="show_heading_text"<?php if ( $this->options['show_heading_text'] ) echo ' checked="checked"'; ?> /><label for="show_heading_text"> <?php _e('Show title on top of the table of contents', 'mjiong-wp-toc'); ?></label><br />
		<div class="more_toc_options<?php if ( !$this->options['show_heading_text'] ) echo ' disabled'; ?>">
			...
			<span class="description"><label for="heading_text"><?php _e('Eg: Contents, Table of Contents, Page Contents', 'mjiong-wp-toc'); ?></label></span><br /><br />
			
			<input type="checkbox" value="1" id="visibility" name="visibility"<?php if ( $this->options['visibility'] ) echo ' checked="checked"'; ?> /><label for="visibility"> <?php _e( 'Allow the user to toggle the visibility of the table of contents', 'mjiong-wp-toc'); ?></label><br />
			<div class="more_toc_options<?php if ( !$this->options['visibility'] ) echo ' disabled'; ?>">
				<table class="more_toc_options_table">
				<tbody>
				<tr>
					<th><label for="visibility_show"><?php _e('Show text', 'mjiong-wp-toc'); ?></label></th>
					...
					_e('Eg: show', 'mjiong-wp-toc'); ?></label></span></td>
				</tr>
				<tr>
					<th><label for="visibility_hide"><?php _e('Hide text', 'mjiong-wp-toc'); ?></label></th>
					...
					_e('Eg: hide', 'mjiong-wp-toc'); ?></label></span></td>
				</tr>
				</tbody>
				</table>
				<input type="checkbox" value="1" id="visibility_hide_by_default" name="visibility_hide_by_default"<?php if ( $this->options['visibility_hide_by_default'] ) echo ' checked="checked"'; ?> /><label for="visibility_hide_by_default"> <?php _e( 'Hide the table of contents initially', 'mjiong-wp-toc'); ?></label>
			</div>
		</div>
	</td>
</tr>
<tr>
	<th><label for="show_heirarchy"><?php _e('Show hierarchy', 'mjiong-wp-toc'); ?></label></th>
...
<tr>
	<th><label for="ordered_list"><?php _e('Number list items', 'mjiong-wp-toc'); ?></label></th>
...
<tr>
	<th><label for="smooth_scroll"><?php _e('Enable smooth scroll effect', 'mjiong-wp-toc'); ?></label></th>
	<td><input type="checkbox" value="1" id="smooth_scroll" name="smooth_scroll"<?php if ( $this->options['smooth_scroll'] ) echo ' checked="checked"'; ?> /><label for="smooth_scroll"> <?php _e( 'Scroll rather than jump to the anchor link', 'mjiong-wp-toc'); ?></label></td>
</tr>
</tbody>
</table>

<h3><?php _e('Appearance', 'mjiong-wp-toc'); ?></h3>
<table class="form-table">
<tbody>
<tr>
	<th><label for="width"><?php _e('Width', 'mjiong-wp-toc'); ?></label></td>
	<td>
		<select name="width" id="width">
			<optgroup label="<?php _e('Fixed width', 'mjiong-wp-toc'); ?>">
			...
			<optgroup label="<?php _e('Relative', 'mjiong-wp-toc'); ?>">
				<option value="Auto"<?php if ( 'Auto' == $this->options['width'] ) echo ' selected="selected"'; ?>><?php _e('Auto (default)', 'mjiong-wp-toc'); ?></option>
			...
			_e('Other', 'mjiong-wp-toc'); ?>">
				<option value="User defined"<?php if ( 'User defined' == $this->options['width'] ) echo ' selected="selected"'; ?>><?php _e('User defined', 'mjiong-wp-toc'); ?></option>
			</optgroup>
		</select>
		<div class="more_toc_options<?php if ( 'User defined' != $this->options['width'] ) echo ' disabled'; ?>">
			...
			printf( __('Please enter a number and %s select its units, eg: 100px, 10em', 'mjiong-wp-toc'), '</label><label for="width_custom_units">' ); ?></label><br />
...
<tr>
	<th><label for="wrapping"><?php _e('Wrapping', 'mjiong-wp-toc'); ?></label></td>
	<td>
		<select name="wrapping" id="wrapping">
			<option value="<?php echo TOC_WRAPPING_NONE; ?>"<?php if ( TOC_WRAPPING_NONE == $this->options['wrapping'] ) echo ' selected="selected"'; ?>><?php _e('None (default)', 'mjiong-wp-toc'); ?></option>
			<option value="<?php echo TOC_WRAPPING_LEFT; ?>"<?php if ( TOC_WRAPPING_LEFT == $this->options['wrapping'] ) echo ' selected="selected"'; ?>><?php _e('Left', 'mjiong-wp-toc'); ?></option>
			<option value="<?php echo TOC_WRAPPING_RIGHT; ?>"<?php if ( TOC_WRAPPING_RIGHT == $this->options['wrapping'] ) echo ' selected="selected"'; ?>><?php _e('Right', 'mjiong-wp-toc'); ?></option>
		</select>
	</td>
</tr>
<tr>
	<th><label for="font_size"><?php _e('Font size', 'mjiong-wp-toc'); ?></label></th>
	<td>
...
<tr>
	...
	_e('Presentation', 'mjiong-wp-toc'); ?></th>
	<td>
		<div class="toc_theme_option">
			<input type="radio" name="theme" id="theme_<?php echo TOC_THEME_GREY; ?>" value="<?php echo TOC_THEME_GREY; ?>"<?php if ( $this->options['theme'] == TOC_THEME_GREY ) echo ' checked="checked"'; ?> /><label for="theme_<?php echo TOC_THEME_GREY; ?>"> <?php _e('Grey (default)', 'mjiong-wp-toc'); ?><br />
			<img src="<?php echo $this->path; ?>/images/grey.png" alt="" />
			</label>
		</div>
		<div class="toc_theme_option">
			<input type="radio" name="theme" id="theme_<?php echo TOC_THEME_LIGHT_BLUE; ?>" value="<?php echo TOC_THEME_LIGHT_BLUE; ?>"<?php if ( $this->options['theme'] == TOC_THEME_LIGHT_BLUE ) echo ' checked="checked"'; ?> /><label for="theme_<?php echo TOC_THEME_LIGHT_BLUE; ?>"> <?php _e('Light blue', 'mjiong-wp-toc'); ?><br />
			<img src="<?php echo $this->path; ?>/images/blue.png" alt="" />
			</label>
		</div>
		<div class="toc_theme_option">
			<input type="radio" name="theme" id="theme_<?php echo TOC_THEME_WHITE; ?>" value="<?php echo TOC_THEME_WHITE; ?>"<?php if ( $this->options['theme'] == TOC_THEME_WHITE ) echo ' checked="checked"'; ?> /><label for="theme_<?php echo TOC_THEME_WHITE; ?>"> <?php _e('White', 'mjiong-wp-toc'); ?><br />
			<img src="<?php echo $this->path; ?>/images/white.png" alt="" />
			</label>
		</div>
		<div class="toc_theme_option">
			<input type="radio" name="theme" id="theme_<?php echo TOC_THEME_BLACK; ?>" value="<?php echo TOC_THEME_BLACK; ?>"<?php if ( $this->options['theme'] == TOC_THEME_BLACK ) echo ' checked="checked"'; ?> /><label for="theme_<?php echo TOC_THEME_BLACK; ?>"> <?php _e('Black', 'mjiong-wp-toc'); ?><br />
			<img src="<?php echo $this->path; ?>/images/black.png" alt="" />
			</label>
		</div>
		<div class="toc_theme_option">
			<input type="radio" name="theme" id="theme_<?php echo TOC_THEME_TRANSPARENT; ?>" value="<?php echo TOC_THEME_TRANSPARENT; ?>"<?php if ( $this->options['theme'] == TOC_THEME_TRANSPARENT ) echo ' checked="checked"'; ?> /><label for="theme_<?php echo TOC_THEME_TRANSPARENT; ?>"> <?php _e('Transparent', 'mjiong-wp-toc'); ?><br />
			<img src="<?php echo $this->path; ?>/images/transparent.png" alt="" />
			</label>
		</div>
		<div class="toc_theme_option">
			<input type="radio" name="theme" id="theme_<?php echo TOC_THEME_CUSTOM; ?>" value="<?php echo TOC_THEME_CUSTOM; ?>"<?php if ( $this->options['theme'] == TOC_THEME_CUSTOM ) echo ' checked="checked"'; ?> /><label for="theme_<?php echo TOC_THEME_CUSTOM; ?>"> <?php _e('Custom', 'mjiong-wp-toc'); ?><br />
		...
		<div class="more_toc_options<?php if ( TOC_THEME_CUSTOM != $this->options['theme'] ) echo ' disabled'; ?>">
			...
			<tbody>
			<tr>
				<th><label for="custom_background_colour"><?php _e('Background', 'mjiong-wp-toc'); ?></label></th>
			...
			<tr>
				<th><label for="custom_border_colour"><?php _e('Border', 'mjiong-wp-toc'); ?></label></th>
			...
			<tr>
				<th><label for="custom_title_colour"><?php _e('Title', 'mjiong-wp-toc'); ?></label></th>
			...
			<tr>
				<th><label for="custom_links_colour"><?php _e('Links', 'mjiong-wp-toc'); ?></label></th>
			...
			<tr>
				<th><label for="custom_links_hover_colour"><?php _e('Links (hover)', 'mjiong-wp-toc'); ?></label></th>
			...
			<tr>
				<th><label for="custom_links_visited_colour"><?php _e('Links (visited)', 'mjiong-wp-toc'); ?></label></th>
			...
			<div class="clear"></div>
			<p><?php printf(__("Leaving the value as %s will inherit your theme's styles", 'mjiong-wp-toc'), '<code>#</code>'); ?></p>
		</div>
	</td>
</tr>
</tbody>
...
<h3><?php _e('Advanced', 'mjiong-wp-toc'); ?> <span class="show_hide">(<a href="#toc_advanced_usage"><?php _e('show', 'mjiong-wp-toc'); ?></a>)</span></h3>
<div id="toc_advanced_usage">
	<h4><?php _e('Power options', 'mjiong-wp-toc'); ?></h4>
	<table class="form-table">
	<tbody>
	<tr>
		<th><label for="lowercase"><?php _e('Lowercase', 'mjiong-wp-toc'); ?></label></th>
		<td><input type="checkbox" value="1" id="lowercase" name="lowercase"<?php if ( $this->options['lowercase'] ) echo ' checked="checked"'; ?> /><label for="lowercase"> <?php _e('Ensure anchors are in lowercase', 'mjiong-wp-toc'); ?></label></td>
	</tr>
	<tr>
		<th><label for="hyphenate"><?php _e('Hyphenate', 'mjiong-wp-toc'); ?></label></th>
		<td><input type="checkbox" value="1" id="hyphenate" name="hyphenate"<?php if ( $this->options['hyphenate'] ) echo ' checked="checked"'; ?> /><label for="hyphenate"> <?php _e('Use - rather than _ in anchors', 'mjiong-wp-toc'); ?></label></td>
	</tr>
	<tr>
		<th><label for="include_homepage"><?php _e('Include homepage', 'mjiong-wp-toc'); ?></label></th>
		<td><input type="checkbox" value="1" id="include_homepage" name="include_homepage"<?php if ( $this->options['include_homepage'] ) echo ' checked="checked"'; ?> /><label for="include_homepage"> <?php _e('Show the table of contents for qualifying items on the homepage', 'mjiong-wp-toc'); ?></label></td>
	</tr>
	<tr>
		<th><label for="exclude_css"><?php _e('Exclude CSS file', 'mjiong-wp-toc'); ?></label></th>
		<td><input type="checkbox" value="1" id="exclude_css" name="exclude_css"<?php if ( $this->options['exclude_css'] ) echo ' checked="checked"'; ?> /><label for="exclude_css"> <?php _e("Prevent the loading of this plugin's CSS styles. When selected, the appearance options from above will also be ignored.", 'mjiong-wp-toc'); ?></label></td>
	</tr>
	<tr>
		<th><label for="bullet_spacing"><?php _e('Preserve theme bullets', 'mjiong-wp-toc'); ?></label></th>
		<td><input type="checkbox" value="1" id="bullet_spacing" name="bullet_spacing"<?php if ( $this->options['bullet_spacing'] ) echo ' checked="checked"'; ?> /><label for="bullet_spacing"> <?php _e('If your theme includes background images for unordered list elements, enable this to support them', 'mjiong-wp-toc'); ?></label></td>
	</tr>
	<tr>
		<th><?php _e('Heading levels', 'mjiong-wp-toc'); ?></th>
		<td>
		<p><?php _e('Include the following heading levels. Deselecting a heading will exclude it.', 'mjiong-wp-toc'); ?></p>
<?php
...
?>
	...
	<tr>
		<th><label for="exclude"><?php _e('Exclude headings', 'mjiong-wp-toc'); ?></label></th>
		<td>
			...
			<label for="exclude"><?php _e('Specify headings to be excluded from appearing in the table of contents.  Separate multiple headings with a pipe <code>|</code>.  Use an asterisk <code>*</code> as a wildcard to match other text.  Note that this is not case sensitive. Some examples:', 'mjiong-wp-toc'); ?></label><br/>
			<ul>
				<li><?php _e('<code>Fruit*</code> ignore headings starting with "Fruit"', 'mjiong-wp-toc'); ?></li>
				<li><?php _e('<code>*Fruit Diet*</code> ignore headings with "Fruit Diet" somewhere in the heading', 'mjiong-wp-toc'); ?></li>
				<li><?php _e('<code>Apple Tree|Oranges|Yellow Bananas</code> ignore headings that are exactly "Apple Tree", "Oranges" or "Yellow Bananas"', 'mjiong-wp-toc'); ?></li>
			</ul>
		</td>
	</tr>
	<tr id="smooth_scroll_offset_tr" class="<?php if ( !$this->options['smooth_scroll'] ) echo 'disabled'; ?>">
		<th><label for="smooth_scroll_offset"><?php _e('Smooth scroll top offset', 'mjiong-wp-toc'); ?></label></th>
		<td>
			...
			<label for="smooth_scroll_offset"><?php _e('If you have a consistent menu across the top of your site, you can adjust the top offset to stop the headings from appearing underneath the top menu. A setting of 30 accommodates the WordPress admin bar. This setting appears after you have enabled smooth scrolling from above.', 'mjiong-wp-toc'); ?></label>
		</td>
	</tr>
	<tr>
		<th><label for="restrict_path"><?php _e('Restrict path', 'mjiong-wp-toc'); ?></label></th>
		<td>
			...
			<label for="restrict_path"><?php _e('Restrict generation of the table of contents to pages that match the required path. This path is from the root of your site and always begins with a forward slash.', 'mjiong-wp-toc'); ?><br />
			<span class="description"><?php 
			/* translators: example URL path restriction */
			_e('Eg: /wiki/, /corporate/annual-reports/', 'mjiong-wp-toc'); ?></span></label>
		</td>
	</tr>
	<tr>
		<th><label for="fragment_prefix"><?php _e('Default anchor prefix', 'mjiong-wp-toc'); ?></label></th>
		<td>
			...
			<label for="fragment_prefix"><?php _e('Anchor targets are restricted to alphanumeric characters as per HTML specification (see readme for more detail). The default anchor prefix will be used when no characters qualify. When left blank, a number will be used instead.', 'mjiong-wp-toc'); ?><br />
			<?php _e('This option normally applies to content written in character sets other than ASCII.', 'mjiong-wp-toc'); ?><br />
			<span class="description"><?php 
			/* translators: example anchor prefixes when no ascii characters match */
			_e('Eg: i, toc_index, index, _', 'mjiong-wp-toc'); ?></span></label>
		</td>
	</tr>
	</tbody>
	</table>
	...
	_e('Usage', 'mjiong-wp-toc'); ?></h4>
	<p><?php printf(__('If you would like to fully customise the position of the table of contents, you can use the %s shortcode by placing it at the desired position of your post, page or custom post type. This method allows you to generate the table of contents despite having auto insertion disabled for its content type. Please visit the help tab for further information about this shortcode.', 'mjiong-wp-toc'), '<code>[toc]</code>'); ?></p>
</div>
...
<p><?php printf(__('At its simplest, placing %s into a page will automatically create a sitemap of all pages and categories. This also works in a text widget.', 'mjiong-wp-toc'), '<code>[sitemap]</code>'); ?></p>
<table class="form-table">
<tbody>
<tr>
	<th><label for="sitemap_show_page_listing"><?php _e('Show page listing', 'mjiong-wp-toc'); ?></label></th>
...
<tr>
	<th><label for="sitemap_show_category_listing"><?php _e('Show category listing', 'mjiong-wp-toc'); ?></label></th>
...
<tr>
	<th><label for="sitemap_heading_type"><?php _e('Heading type', 'mjiong-wp-toc'); ?></label></th>
	...
	_e('Use', 'mjiong-wp-toc'); ?> h</label><select name="sitemap_heading_type" id="sitemap_heading_type">
<?php
			// h1 to h6
			for ($i = 1; $i <= 6; $i++) {
				echo '<option value="' . $i . '"';
...
?>
		...
		_e('to print out the titles', 'mjiong-wp-toc'); ?>
	</td>
</tr>
<tr>
	<th><label for="sitemap_pages"><?php _e('Pages label', 'mjiong-wp-toc'); ?></label></th>
	<td><input type="text" class="regular-text" value="<?php echo htmlentities( $this->options['sitemap_pages'], ENT_COMPAT, 'UTF-8' ); ?>" id="sitemap_pages" name="sitemap_pages" />
		<span class="description"><?php _e('Eg: Pages, Page List', 'mjiong-wp-toc'); ?></span>
	</td>
</tr>
<tr>
	<th><label for="sitemap_categories"><?php _e('Categories label', 'mjiong-wp-toc'); ?></label></th>
	<td><input type="text" class="regular-text" value="<?php echo htmlentities( $this->options['sitemap_categories'], ENT_COMPAT, 'UTF-8' ); ?>" id="sitemap_categories" name="sitemap_categories" />
		<span class="description"><?php _e('Eg: Categories, Category List', 'mjiong-wp-toc'); ?></span>
	</td>
</tr>
</tbody>
</table>

<h3><?php _e('Advanced usage', 'mjiong-wp-toc'); ?> <span class="show_hide">(<a href="#sitemap_advanced_usage"><?php _e('show', 'mjiong-wp-toc'); ?></a>)</span></h3>
<div id="sitemap_advanced_usage">
	<p><code>[sitemap_pages]</code> <?php printf(__('lets you print out a listing of only pages. Similarly %s can be used to print out a category listing. They both can accept a number of attributes so visit the help tab for more information.', 'mjiong-wp-toc'), '<code>[sitemap_categories]</code>'); ?></p>
	<p><?php _e('Examples', 'mjiong-wp-toc'); ?></p>
	<ol>
		<li><code>[sitemap_categories no_label="true"]</code> <?php _e('hides the heading from a category listing', 'mjiong-wp-toc'); ?></li>
		<li><code>[sitemap_pages heading="6" label="This is an awesome listing" exclude="1,15"]</code> <?php printf(__('Uses h6 to display %s on a page listing excluding pages with IDs 1 and 15', 'mjiong-wp-toc'), '<em>This is an awesome listing</em>'); ?></li>
	</ol>
</div>
...
<p class="submit"><input type="submit" name="submit" class="button-primary" value="<?php _e('Update Options', 'mjiong-wp-toc'); ?>" /></p>
</form>
</div>
<?php
		}
	...
	class toc_widget extends WP_Widget {
		...
		function __construct()
		{
			$widget_options = array( 
				...
				'description' => __('Display the table of contents in the sidebar with this widget', 'mjiong-wp-toc') 
			);
			...
			parent::__construct( 'toc-widget', 'MJIONG WP-TOC', $widget_options, $control_options );
		}
...
?>
			<p>
				<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e('Title', 'mjiong-wp-toc'); ?>:</label>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id( 'hide_inline' ); ?>"> <?php _e('Show the table of contents only in the sidebar', 'mjiong-wp-toc'); ?></label>
			</p>
			...
			<div class="show_toc_in_widget_only_post_types" style="margin: 0 0 25px 25px; display: <?php echo ( $toc_options['show_toc_in_widget_only'] == 1 ) ? 'block' : 'none'; ?>;">
				<p><?php _e('For the following content types:', 'mjiong-wp-toc'); ?></p>
